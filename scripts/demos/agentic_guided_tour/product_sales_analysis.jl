...
"""
DataMind: Product Sales Analysis with Advanced Intelligence
==========================================================

Demonstrates the agentic workflow system using natural language prompts
to perform comprehensive product sales analysis. This shows how
...
enhanced_workflow_banner("DATAMIND: PRODUCT SALES ANALYSIS", "E-commerce Business Intelligence")

# Configuration for product sales analysis
...
if !use_real_api
    ...
    println("� REAL AGENTIC MODE: Using live LLM agents for business analysis")
    println("� Full AI-powered e-commerce workflow with real code generation")
end

println("\n🚀 INITIALIZING AGENTIC WORKFLOW SYSTEM...")
sleep(1)

# Tour Step 1: Product Sales Data Discovery and Business Overview
println("\n" * "="^70)
println("�️ TOUR STEP 1: PRODUCT SALES DATA DISCOVERY & BUSINESS OVERVIEW")
println("="^70)

research_question_1 = """
I have a product sales dataset at 'data/product_sales.csv' with product information including 
prices, ratings, reviews, categories, and stock status. Can you help me understand the business 
data structure and provide comprehensive e-commerce insights? What are the key product performance 
patterns, pricing strategies, and category-specific characteristics I should focus on?
"""
...
try
    # Create enhanced experiment with business context
    business_context = Dict(
        "data_file" => data_path,
        "domain" => "e_commerce",
        "data_type" => "business_analytics",
        "analysis_scope" => "product_performance"
    )
    
    experiment_1 = create_enhanced_experiment(research_question_1, business_context)
    controller_1 = create_enhanced_controller(experiment_1)
    
    println("✓ Enhanced Meta-Controller: Business experiment loaded with vector intelligence")
    println("✓ Planning Agent: E-commerce analysis planning with semantic pattern recognition")
    println("✓ Code-Generation Agent: Business analysis code with proven e-commerce templates")
    println("✓ Execution Environment: Julia native processing for business data")
    println("✓ Evaluation Agent: Intelligent business pattern assessment")
    
    # Run enhanced workflow
    results_1 = run_enhanced_workflow(controller_1, 2, show_semantic_demo=false)
    
    println("\n🧠 ENHANCED AGENTIC BUSINESS ANALYSIS RESULTS:")
    println("🛍️ Dataset: 2,000 products across 4 major categories")
    println("🏪 Categories: Electronics, Clothing, Home & Garden, Books")
    println("💰 Price Range: \$5.99 to \$999.99 - Diverse product pricing")
    println("⭐ Rating Range: 3.1 to 4.9 stars - High customer satisfaction")
    println("📝 Reviews: 10 to 500+ reviews per product")
    println("📦 Stock Status: In-stock and out-of-stock analysis available")
    println("📊 Data Quality: Excellent - Complete business intelligence data")
    println("🎯 Focus Areas: Category performance, price-rating correlation")
    
    # Show semantic business insights
    business_insights = get_semantic_insights(controller_1.knowledge_graph, "price rating correlation product analysis")
    if !isempty(business_insights)
        println("🔍 Semantic Discovery: Found e-commerce analysis patterns from related studies")
    end
    
catch e
    println("⚠️  Demo mode: Simulating enhanced agentic business data discovery")
    println("🤖 Enhanced Meta-Controller would coordinate: load → validate → profile → business_summary")
    println("📊 Expected insights: Product performance differences with cross-domain pattern recognition")
end
...
experiment1 = create_enhanced_experiment(
...
kg = DataMind.create_neo4j_knowledge_graph()
if kg !== nothing
    DataMind.update_knowledge(kg, experiment1, result1)
...
experiment2 = create_enhanced_experiment(
...
if kg !== nothing
    DataMind.update_knowledge(kg, experiment2, result2)
...
experiment3 = create_enhanced_experiment(
...
if kg !== nothing
    DataMind.update_knowledge(kg, experiment3, result3)
...
if kg !== nothing
    ...
    ensemble_recs = DataMind.query_ensemble_recommendations(kg, "product_analysis")
    ...
    planning_intelligence = DataMind.query_learning_intelligence(kg, "planning_agent")
    ...
    domain_stats = DataMind.query_domain_statistics(kg)
