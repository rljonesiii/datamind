...
begin
	...
	# Set plotting backend with fallback
	try
		using PlotlyJS
		plotlyjs()
	catch
		# Fallback to GR if PlotlyJS fails
		gr()
	end
	
	# Simple data loading function
	function load_and_prepare_data_simple(file_path; validate=true)
		...
		if validate && nrow(df) == 0
			throw(ArgumentError("Dataset is empty after cleaning"))
		end
...
**Interactive Pluto Notebook with Julia Native Performance**

This dashboard demonstrates:
- 📊 **Interactive visualizations** with real-time updates
- 🚀 **High-performance Julia processing** 
- 💡 **Business intelligence insights** with actionable recommendations

*Powered by Julia native ecosystem and Pluto reactivity*
"""
...
Show 3D Plot: $(@bind enable_3d CheckBox(default=false))
"""

# ╔═╡ 4b9e8f90-4567-89ab-def0-12345678cdef
begin
	Random.seed!(random_seed)
	md"🎲 Random seed set to: $random_seed"
end

# ╔═╡ 5a1f2b90-5abc-def0-1234-56789abcdef0
begin
	# Load credit card data
	df = try
		# Try different possible paths for the data file
		data_paths = [
			"../data/cc_data.csv",  # From notebooks directory
			"data/cc_data.csv",     # From project root
			joinpath(@__DIR__, "..", "data", "cc_data.csv")  # Absolute path
		]
		...
		if data_file === nothing
			error("Could not find cc_data.csv in any expected location")
		end
		
		temp_df = load_and_prepare_data_simple(data_file, validate=true)
	...
	if nrow(df) > 0
		...
		**📋 Dataset Overview:**
		...
		md"""
		⚠️ **Data Loading Error:** Could not load cc_data.csv
...
begin
	if nrow(df) > 0
		...
		md"❌ No data available for risk assessment"
	end
end
...
begin
	if nrow(df) > 0
		...
		p1 = Plots.bar(risk_counts.RISK_CATEGORY, risk_counts.count,
		...
		for (i, row) in enumerate(eachrow(risk_counts))
			Plots.annotate!(p1, (i, row.count + 50, Plots.text("$(row.percentage)%", 10, :center)))
		end
		
		p1
	else
		Plots.plot(title="No data available", size=(400, 300))
	end
end
...
begin
	if nrow(df) > 0
		...
		# Get top customers - SINGLE DEFINITION
		top_customers = sort(df, :ESTIMATED_CLV, rev=true)[1:min(top_n_customers, nrow(df)), :]
		
		# Calculate all business metrics here
		...
		roi_opportunity = round(potential_revenue * 0.15, digits=2)
		
		md"""
		**💎 Customer Value Insights:**
		- **Average CLV:** \$$avg_clv per customer
		- **Top $(min(top_n_customers, nrow(df))) CLV:** \$$(round(sum(top_customers.ESTIMATED_CLV), digits=2))
		- **Portfolio Value:** \$$(total_portfolio_value)M total balance
		- **High Risk Customers:** $high_risk_customers ($high_risk_percentage%)
		- **Premium Customers:** $high_value_customers
		"""
	else
		...
		md"❌ No data available for value analysis"
	end
end
...
begin
	if nrow(df) > 0
		# Customer value vs risk plot
		if enable_3d
			# 3D scatter plot
			Plots.scatter(df.ESTIMATED_CLV, df.CREDIT_RISK_SCORE, df.BALANCE,
			...
			# 2D scatter plot
			Plots.scatter(df.ESTIMATED_CLV, df.CREDIT_RISK_SCORE,
				group=df.RISK_CATEGORY,
...
begin
	if nrow(df) > 0
		# Use metrics calculated in the Customer Value Analysis cell
		HTML("""
		<div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
			...
			<div style="margin-top: 20px; padding: 15px; background: rgba(255,255,255,0.1); border-radius: 8px;">
				...
				<p><strong>Top $(min(top_n_customers, nrow(df))) Customers Value:</strong> \$$(round(potential_revenue, digits=0))</p>
				<p><strong>Annual Improvement Potential:</strong> \$$roi_opportunity</p>
				<p><em>Julia Native Performance: Instant real-time analytics</em></p>
			</div>
		</div>
		""")
	else
		...
		<div style="background: #f44336; color: white; padding: 20px; border-radius: 10px;">
			...
			<p>Could not load credit card data. Please ensure data/cc_data.csv exists.</p>
		</div>
		""")
	end
end
...
manifest_format = "2.0"
project_hash = "c63c6af23265a40389fa3297fb30e80220537852"
